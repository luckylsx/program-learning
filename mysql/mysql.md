...
**导致索引失效的有哪些**
- 计算 如： +、-、*、/、!=、<>、is_null、is not null、or
- 函数 sum(), round() 等等
- 手动/自动类型转换， 如：id="1" 本来是数字给 写成了字符串
...
- 读未提交， 一个事务未提交时 他所做的变更就能被别的事务看到
- 读提交 ， 一个事务提交之后 他所做的变更才能被别的事务看到
- 可重复读
- 串行化 读加读锁 写加写锁，当出现读写冲突时候，后访问的事务必须等前一个事务执行完成，才能继续执行

读未提交的隔离级别 出现 脏读 解决方法 读提交

读提交 出现 不可重复读 解决方法 可重复读

可重复度 出现 幻读 读加读锁 写加写锁，会阻止 update 和 delete 但是不包括insert, 会出现第一次读取到10条记录，第二次读取到11条记录 解决方法 串行化
...
b+数是按照从左到右的顺序来建立搜索树的，比如当(张三,20,F)这样的数据来检索的时候，b+树会优先比较name来确定下一步的搜索方向，如果name相同再依次比较age和sex，最后得到检索的数据；但当(20,F)这样的没有name的数据来的时候，b+树就不知道下一步该查哪个节点，因为建立搜索树的时候name就是第一个比较因子，必须要先根据name来搜索才能知道下一步去哪里查询。
...
### 48. mysql 行锁，表锁 间隙锁的提现。

- 行锁：
> 一个事务在对一行数据进行更新的时候，另一个事务也同时对该行数据进行更新，在第一个事务没提交对时候，第二个事务更新的时候会被阻塞

- 表级锁：
> 行锁失效行锁升级为表锁， 用or 做连接会导致索引失效，行锁会升级为表锁。 在一个事务中更新 id 为1 的记录，另一个事务中更新 id 为 2 的进程会被阻塞。

- 间隙锁
> 

### 49. mysql 查询 or 的注意事项：
1. 使用同一列带索引的字段进行查询的时候 or 与 in 相同 都可以用到索引
2. 使用不同列带索引字段进行查询 这样的情况下也会使用到索引
3. 如果or 中有个where 条件不带索引，那么sql 就不会走索引了
...
会有5个参数：
- innodb_row_lock_current_waits: 多少个sql 在等待锁
...
2. 使用左匹配原则查询
3. 索引不要放在范围查询后面
4. 减少select * 的使用 使用不到覆盖索引，也会浪费网络带宽
5. order by 的优化

### 52. innodb 引擎数据的存储结构

两部分存储：.frm 存储表的数据结构 .ibd存储的是数据
